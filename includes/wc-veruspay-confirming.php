...
<div class="wc_veruspay_payment_container">
    ...
    <div id="wc-<?php echo esc_attr( $wc_veruspay_class->id ); ?>-vrsc-form" class="wc-payment-form" style="background:transparent;">
        <pre class="wc_veruspay_address" id="verusAddress"><?php if ( substr( $wc_veruspay_address, 0, 2 ) !== 'zs' ) { echo '<a style="color:#fff!important" target="_BLANK" href="' . $wc_veruspay_global['chain_dtls'][strtolower( $wc_veruspay_coin )]['address'] . $wc_veruspay_address . '">' . $wc_veruspay_address . '</a>'; } else { echo $wc_veruspay_address; } ?></pre>
    </div>
</div>
